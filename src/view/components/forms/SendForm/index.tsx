...
const TransferFunds: FC<IProps> = ({
  ...
  const [sendingErrors, setErrors] = useState({ amount: false, to: false, password: false });
  ...
  const renderSendFormSucess = (isSendSucess, errorType, publicAddress, onClick) => {
  ...
  return (
    <>
      {renderSendFormSucess(isSendSucess, errorType, data.publicAddress, onClick)}
      ...
      >
        <ModalBody className={styles.body}>
          <Input
            ...
            errorMsg={(errorType === "password" || sendingErrors.password)? "Invalid password": ''}
          />
      ...
      {errorType === ''&& !isSendSucess && <div className={classnames('card', styles.card)}>
