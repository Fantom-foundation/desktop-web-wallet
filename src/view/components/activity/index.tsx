      ...
      <div className={styles.activitiesRow} onClick={() => setIsOpen(!isOpen)}>
        ...
        >
          ...
          <p className={styles.ftm}> 
            {' '}
            {isRecieve ? '-' : '+'}
            {props.data.value && parseFloat(Web3.utils.fromWei(props.data.value)).toFixed(5)} 
            {' '}
...
FTM
      ...
      <SubView  to={props.data.to} fee={props.data.fee} newDate={newDate} value={props.data.hash} />
      </Collapse>
    </div>
  );
};
export default props => {
  ...
  return (<Card>
    ...
    <div>
      ...
      {props.transactions.list && props.transactions.list.length > 0 && props.transactions.list.map((data: object, index: number) => (
